...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    ...
                    screenshot_filename = f"Teja_screenshots(6)/Guru_Teja_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
        ...
        if (datetime.now() - last_saved_time).total_seconds() >= 30:
            df.to_excel('teja_attendance_with_screenshots_dt(6).xlsx', index=False)
            print("Data saved to 'teja_attendance_with_screenshots_dt(6).xlsx'.")
    ...
    if not df.empty:
        df.to_excel('teja_attendance_with_screenshots_dt(6).xlsx', index=False)
        print("Attendance and screenshots saved to 'teja_attendance_with_screenshots_dt(6).xlsx'.")
